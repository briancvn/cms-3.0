...
use CMS\Infrastructure\Extension\Cache\ActionCache;
use CMS\Infrastructure\Extension\Cache\ControllerCache;
...
class Route {
    ...
    public function __construct(Api $app, \Phalcon\DiInterface $di) {
        $this->app = $app;
        $this->di = $di;
    ...
    protected function autoloadClass($className) {
        ...
        if (!$this->ourLoader) {
            $this->ourLoader = new \Phalcon\Loader();
        ...
        if (!$loaded) {
            throw new \Phalcon\Exception('Unable to load autoload class ' . $className);
        }
    }
            ...
            foreach ($scan as $f) {
                ...
                foreach ($classRef->getMethods() as $methodRef) {
                    ...
                    if (!$ignore) {
                        ...
                        $paramType = empty($parameters) || $parameters[0] == null || $parameters[0]->getType() == null ? null : $parameters[0]->getType()->getName();
                        if ($controllerCache) {
    ...
    protected function updateCollection($className, $classMethod, $uri, $httpMethod) {
        if (!isset($this->collections[$className])) {
            $this->collections[$className] = new \Phalcon\Mvc\Micro\Collection();
        ...
        $this->collections[$className]->setPrefix('/'.MODULE);
